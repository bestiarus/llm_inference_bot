...
class DialogueTracker:
    ...
    def __init__(self, seconds_to_reset: float = 60 * 60, messages_in_history: int = None):
        logger.info(
            f"Initializing Groq-based model '{self._MODEL_NAME}'. Seconds to clear history: {seconds_to_reset}, max messages per history: {messages_in_history}"
        )
        groq_api_key = getenv(self._GROQ_API_KEY)
